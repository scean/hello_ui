...
class StorageManager {
        ...
        switch (destination) {
            ...
            case Downloads.Impl.DESTINATION_FILE_URI:
                if (path.startsWith(mExternalStorageDir.getPath())) {
                    ...
                    if (!TextUtils.isEmpty(secondaryStoragePath) && path.startsWith(secondaryStoragePath)) {
                        dir = new File(secondaryStoragePath);
                    }
                }
                break;
         }
    ...
    private void removeSpuriousFiles() {
        ...
        Cursor cursor = mContext.getContentResolver().query(
                Downloads.Impl.ALL_DOWNLOADS_CONTENT_URI,
                new String[] { Downloads.Impl._DATA, Downloads.Impl.COLUMN_STATUS }, null, null, null);
        try {
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    ...
                    if (!TextUtils.isEmpty(filename)) {
                        ...
                        if (status != Downloads.Impl.STATUS_SUCCESS) {
                            filename = filename + Helpers.sDownloadingExtension;
                        }
